...
type AnalyticsPosition={name:string,quantity:number,revenue:number,coffeePortions:number,decafPackets:number};
type Analytics={period:AnalyticsPeriod,periodDays:number,revenue:number,ordersCount:number,averageCheck:number,cash:number,card:number,coffeePortions:number,decafPackets:number,positions:AnalyticsPosition[],daily:{date:string,revenue:number,ordersCount:number}[]};
...
function App(){
 ...
 const [pdfReport,setPdfReport]=useState<{fileName:string,pageCount:number,text:string}|null>(null);
 ...
 const uploadDailyPdf=async(event:React.ChangeEvent<HTMLInputElement>)=>{const file=event.target.files?.[0];if(!file)return;const form=new FormData();form.append('file',file);try{setPdfReport(await api<{fileName:string,pageCount:number,text:string}>('/api/analytics/daily-pdf',{method:'POST',body:form}));setToast('PDF загружен')}catch(error:any){setToast(error.message)}event.target.value=''};
