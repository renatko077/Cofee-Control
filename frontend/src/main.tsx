...
import {ArrowRight,BarChart3,Check,Clock,Coffee,Download,Minus,MoreHorizontal,Plus,RefreshCw,ShoppingBag,Trash2,Wallet,X} from 'lucide-react';
import './style.css';
...
function App(){
 ...
 const downloadDailyPdf=async()=>{setReportBusy(true);try{const report=await api<Analytics>(`/api/analytics?period=today&date=${reportDate}`);const [{default:pdfMake},{default:pdfFonts}]=await Promise.all([import('pdfmake/build/pdfmake'),import('pdfmake/build/vfs_fonts')]);(pdfMake as any).vfs=(pdfFonts as any).vfs;const formattedDate=new Date(`${reportDate}T00:00:00`).toLocaleDateString('ru-RU',{day:'2-digit',month:'long',year:'numeric'});const rows=report.positions.length?report.positions.map((position,index)=>[{text:String(index+1),alignment:'center'},{text:position.name},{text:String(position.quantity),alignment:'center',bold:true}]):[[{text:'Продаж за этот день нет',colSpan:3,alignment:'center',color:'#978b82'}, {}, {}]];const documentDefinition:any={pageSize:'A4',pageMargins:[42,44,42,44],defaultStyle:{font:'Roboto',fontSize:10,color:'#29211d'},content:[{columns:[{stack:[{text:'COFFEE CONTROL',fontSize:10,bold:true,color:'#8a6044',characterSpacing:1.5},{text:'Дневной отчёт',fontSize:24,bold:true,margin:[0,5,0,3]},{text:formattedDate,color:'#756a63'}]},{text:'DAY REPORT',fontSize:9,bold:true,alignment:'right',color:'#8a6044',characterSpacing:1.2}],margin:[0,0,0,24]},{table:{widths:['*','*','*','*'],body:[[{text:'Чашки',style:'metricLabel'},{text:'Порции кофе',style:'metricLabel'},{text:'Гриндер',style:'metricLabel'},{text:'Без кофеина',style:'metricLabel'}],[{text:String(report.cups),style:'metricValue'},{text:String(report.coffeePortions),style:'metricValue'},{text:String(report.grinderPortions),style:'metricValue'},{text:String(report.decafCoffees),style:'metricValue'}]]},layout:{fillColor:(row:number)=>row===0?'#f1e8e1':'#fbf8f5',hLineColor:()=> '#e2d6cd',vLineColor:()=> '#e2d6cd',paddingTop:()=>10,paddingBottom:()=>10,paddingLeft:()=>8,paddingRight:()=>8},margin:[0,0,0,24]},{text:'Купленные позиции',fontSize:16,bold:true,margin:[0,0,0,10]},{table:{headerRows:1,widths:[28,'*',58],body:[[{text:'№',style:'tableHeader',alignment:'center'},{text:'Позиция',style:'tableHeader'},{text:'Кол-во',style:'tableHeader',alignment:'center'}],...rows]},layout:{fillColor:(row:number)=>row===0?'#6f4e37':row%2===0?'#faf6f2':null,hLineColor:()=> '#e8ded6',vLineColor:()=> '#e8ded6',paddingTop:()=>8,paddingBottom:()=>8,paddingLeft:()=>8,paddingRight:()=>8},margin:[0,0,0,22]},{columns:[{text:`Заказов: ${report.ordersCount}`,color:'#756a63'},{text:`Выручка: ${money(report.revenue)}`,alignment:'right',bold:true,color:'#6f4e37'}]},{text:'Гриндер считает только обычный кофе. Напитки без кофеина вынесены отдельно и в гриндер не входят.',fontSize:8,color:'#978b82',margin:[0,18,0,0]}],footer:(currentPage:number,pageCount:number)=>({columns:[{text:'Coffee Control',color:'#a1948b'},{text:`${currentPage} / ${pageCount}`,alignment:'right',color:'#a1948b'}],fontSize:8,margin:[42,0,42,20]}),styles:{metricLabel:{fontSize:8,bold:true,color:'#756a63',alignment:'center'},metricValue:{fontSize:20,bold:true,color:'#6f4e37',alignment:'center'},tableHeader:{bold:true,color:'#ffffff'}}};pdfMake.createPdf(documentDefinition).download(`coffee-report-${reportDate}.pdf`);setToast('PDF-отчёт скачан')}catch(error:any){setToast(error.message||'Не удалось создать PDF')}finally{setReportBusy(false)}};
