...
type AnalyticsPosition={name:string,quantity:number,modifiers?:{name:string,quantity:number}[]};
...
const createDailyPdf=async(report:Analytics,date:string)=>{
 const positions=report.positions.length?report.positions.map((position,index)=>[{text:String(index+1),alignment:'center'},{stack:[{text:position.name},{text:position.modifiers?.length?`Добавки: ${position.modifiers.map(modifier=>`${modifier.name}${modifier.quantity>1?` ×${modifier.quantity}`:''}`).join(', ')}`:'',fontSize:8,color:'#8a6044',margin:[0,3,0,0]}]},{text:String(position.quantity),alignment:'center',bold:true}]):[[{text:'Продаж за этот период нет',colSpan:3,alignment:'center',color:'#978b82'}, {}, {}]];
 ...
 const chartSection=report.periodDays>1?[{text:'Продажи по дням',fontSize:16,bold:true,margin:[0,0,0,4]},{text:'Число над столбцом — количество заказов, снизу — день месяца',fontSize:8,color:'#897d74',margin:[0,0,0,6]},daily.length?{svg:chartSvg,width:500,margin:[0,0,0,14]}:{text:'Продаж за период нет',color:'#978b82',margin:[0,10,0,20]},{text:`Всего заказов: ${report.ordersCount}`,bold:true,color:'#6f4e37',margin:[0,0,0,20]}]:[{text:`Всего заказов: ${report.ordersCount}`,bold:true,color:'#6f4e37',margin:[0,0,0,20]}];
...
function App(){
 ...
 useEffect(()=>{
  ...
  Promise.all([refresh('today'),api<Product[]>('/api/products').then(setProducts)]).catch(error=>setToast(error.message)).finally(()=>setLoading(false));
 },[refresh]);
 ...
 return <div className="app">
  ...
  {tab==='home'&&<main>
   ...
   {!dashboard?.currentShift?<section className="open-card"><Clock size={32}/><h2>Смена ещё не открыта</h2><p>Укажите наличные в кассе, чтобы начать работу.</p><input inputMode="decimal" placeholder="Наличные на начало, ₴" value={openingCash} onChange={event=>setOpeningCash(event.target.value)}/><button className="primary" onClick={openShift} disabled={busy}>{busy?'Открываем…':'Открыть смену'} <ArrowRight size={18}/></button></section>:<>
    ...
    <button className="primary big" onClick={()=>{setCategory('Кофе');setTab('order')}}><Plus/> Новый заказ</button>
  ...
  {tab!=='settings'&&<nav><button className={tab==='home'?'active':''} onClick={()=>setTab('home')}><Wallet/>Главная</button><button className={tab==='orders'?'active':''} onClick={()=>setTab('orders')}><ShoppingBag/>Заказы</button><button className="add" aria-label="Создать заказ" onClick={()=>{setCategory('Кофе');setTab('order')}}><Plus strokeWidth={2.4}/></button><button className={tab==='analytics'?'active':''} onClick={()=>setTab('analytics')}><BarChart3/>Аналитика</button><button className={tab==='more'?'active':''} onClick={()=>setTab('more')}><MoreHorizontal/>Ещё</button></nav>}
 </div>;
}
