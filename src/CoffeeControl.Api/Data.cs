...
{
 public DbSet<User> Users => Set<User>(); public DbSet<Shift> Shifts => Set<Shift>(); public DbSet<ProductCategory> Categories => Set<ProductCategory>(); public DbSet<Product> Products => Set<Product>(); public DbSet<ProductVariant> Variants => Set<ProductVariant>(); public DbSet<Order> Orders => Set<Order>(); public DbSet<OrderItem> OrderItems => Set<OrderItem>(); public DbSet<OrderItemModifier> OrderItemModifiers => Set<OrderItemModifier>(); public DbSet<Payment> Payments => Set<Payment>(); public DbSet<WriteOff> WriteOffs => Set<WriteOff>();
...
public static class DbSeeder
{
    public static async Task SeedAsync(AppDbContext db, CancellationToken ct)
    {
        ...
        var items = new (string Name, string Category, decimal Price, string Variant, int? Volume, bool Quick)[]
        {
            ...
            ("Лимонад", "Вода", 95, "Стандарт", null, true),
            ("Лимонад Моршинська", "Вода", 35, "Стандарт", null, false),
            ("Shake", "Вода", 30, "Стандарт", null, false),
            ("Mojo", "Вода", 30, "Стандарт", null, false),
            ("Сок", "Вода", 25, "Стандарт", null, false),
            ("Мохито", "Вода", 120, "Стандарт", null, false),
            ...
            ("Pepsi", "Вода", 35, "330 мл", 330, false),
            ("Fanta", "Вода", 35, "330 мл", 330, false),
            ("Coca-Cola", "Вода", 35, "330 мл", 330, false),
            ("Coca-Cola Zero", "Вода", 35, "330 мл", 330, false),
            ("Red Bull", "Вода", 30, "250 мл", 250, false),
            ("Burn", "Вода", 55, "250 мл", 250, false),
            ("Non Stop Original 250 мл", "Вода", 40, "250 мл", 250, false),
            ("Non Stop Original 500 мл", "Вода", 60, "500 мл", 500, false),
            ("Extra Life", "Вода", 50, "500 мл", 500, false),
            // Старая позиция «Вода» за 45 грн отключается ниже, поэтому в новое меню не добавляется.
        ...
        await db.SaveChangesAsync(ct);
        var legacyWater = await db.Products.FirstOrDefaultAsync(x => x.Name == "Вода", ct);
        if (legacyWater is not null) legacyWater.IsActive = false;
        await db.SaveChangesAsync(ct);
    }
}
